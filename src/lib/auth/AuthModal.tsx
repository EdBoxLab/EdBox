

import React from 'react';
import { useAuth } from './AuthContext';

interface AuthModalProps {
  onClose: () => void;
}

const GoogleIcon = () => (
    <svg className="w-5 h-5 mr-3" viewBox="0 0 48 48">
        <path fill="#FFC107" d="M43.611,20.083H42V20H24v8h11.303c-1.649,4.657-6.08,8-11.303,8c-6.627,0-12-5.373-12-12c0-6.627,5.373-12,12-12c3.059,0,5.842,1.154,7.961,3.039l5.657-5.657C34.046,6.053,29.268,4,24,4C12.955,4,4,12.955,4,24c0,11.045,8.955,20,20,20c11.045,0,20-8.955,20-20C44,22.659,43.862,21.35,43.611,20.083z"></path>
        <path fill="#FF3D00" d="M6.306,14.691l6.571,4.819C14.655,15.108,18.961,12,24,12c3.059,0,5.842,1.154,7.961,3.039l5.657-5.657C34.046,6.053,29.268,4,24,4C16.318,4,9.656,8.337,6.306,14.691z"></path>
        <path fill="#4CAF50" d="M24,44c5.166,0,9.86-1.977,13.409-5.192l-6.19-5.238C29.211,35.091,26.715,36,24,36c-5.222,0-9.619-3.317-11.283-7.946l-6.522,5.025C9.505,39.556,16.227,44,24,44z"></path>
        <path fill="#1976D2" d="M43.611,20.083H42V20H24v8h11.303c-0.792,2.237-2.231,4.166-4.087,5.574l6.19,5.238C39.99,35.508,44,30.028,44,24C44,22.659,43.862,21.35,43.611,20.083z"></path>
    </svg>
);

const GitHubIcon = () => (
    <svg className="w-5 h-5 mr-3" fill="currentColor" viewBox="0 0 24 24">
        <path fillRule="evenodd" d="M12 2C6.477 2 2 6.477 2 12c0 4.418 2.865 8.168 6.839 9.492.5.092.682-.217.682-.482 0-.237-.009-.868-.014-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.031-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.82c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.203 2.398.1 2.651.64.7 1.03 1.595 1.03 2.688 0 3.848-2.338 4.695-4.566 4.942.359.308.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.001 10.001 0 0022 12c0-5.523-4.477-10-10-10z" clipRule="evenodd"></path>
    </svg>
);


export const AuthModal: React.FC<AuthModalProps> = ({ onClose }) => {
    const { signInWithGoogle, signInWithGithub, setIsGuest } = useAuth();

    const handleSignIn = async (provider: 'Google' | 'GitHub' | 'Guest') => {
        try {
            if (provider === 'GitHub') {
                await signInWithGithub();
            } else if (provider === 'Google') {
                await signInWithGoogle();
            } else {
                setIsGuest(true);
            }
            onClose();
        } catch (error) {
            console.error("Sign-in failed:", error);
            // You could show an error message to the user here
        }
    };

    return (
        <div 
            className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 animate-fade-in"
            onClick={onClose}
        >
            <div 
                className="bg-white dark:bg-slate-800 rounded-xl p-8 border border-slate-200 dark:border-slate-700 shadow-2xl w-full max-w-sm"
                onClick={e => e.stopPropagation()}
            >
                <div className="text-center">
                    <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-2">Sign In to EdBox</h2>
                    <p className="text-slate-500 dark:text-slate-400 mb-6">Connect your GitHub account to start coding, or continue as a guest.</p>
                </div>
                <div className="space-y-4">
                     <button
                        onClick={() => handleSignIn('GitHub')}
                        className="w-full flex items-center justify-center bg-[#24292e] text-white py-2.5 px-4 rounded-md font-semibold hover:bg-[#333] transition-colors"
                    >
                        <GitHubIcon />
                        Sign in with GitHub
                    </button>
                    <button
                        onClick={() => handleSignIn('Google')}
                        className="w-full flex items-center justify-center bg-white text-slate-700 py-2.5 px-4 rounded-md font-semibold hover:bg-slate-200 transition-colors border border-slate-300"
                    >
                        <GoogleIcon />
                        Sign in with Google
                    </button>
                </div>
                 <div className="mt-6 text-center">
                    <button onClick={() => handleSignIn('Guest')} className="text-sm text-slate-500 hover:text-slate-700 dark:hover:text-slate-300">
                        Continue as Guest
                    </button>
                </div>
            </div>
        </div>
    );
};